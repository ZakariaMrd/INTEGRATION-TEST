...
class CalculerAireTest {
    ...
    public void testAireWithDifferentFormes() {
        ...
        when(carreMock.aire()).thenReturn(4.0);

        CalculerAire calculerAire = new CalculerAire();
        List<FormeFacade> formes = List.of(mockForme1, mockForme2, carreMock);

        double result = calculerAire.aire(formes);
        assertEquals(23.0, result, 0.0001); // Adjust the expected value based on your logic
    }
